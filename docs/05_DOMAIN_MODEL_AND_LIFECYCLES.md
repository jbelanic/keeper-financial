...
- An exact assignment with no required issued policy versions satisfies the derived `policy_acknowledgement` gate without creating fabricated acknowledgement evidence. Any assigned required version remains blocking until that exact assignment/version is acknowledged.
...
- The `agent` role is prerequisite authorization configuration. Migration and local seed paths configure the role definition without granting it to any user; only successful explicit completion grants that existing role to the exact active candidate user.
