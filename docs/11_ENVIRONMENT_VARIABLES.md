...
| Malware scan | `MALWARE_SCANNER_BACKEND`, `MALWARE_SCANNER_FAIL_CLOSED`, `CLAMAV_HOST`, `CLAMAV_PORT`, `CLAMAV_CONNECT_TIMEOUT_SECONDS`, `CLAMAV_READ_TIMEOUT_SECONDS`, `CLAMAV_MAX_CHUNKS`, `CLAMAV_MAX_RESPONSE_BYTES` | Live requires `clamav` at internal `clamav:3310`, bounded timeouts/chunks/responses, and fail-closed behavior. Compose publishes 3310 only on loopback for verification. `local_test` is test plumbing and is rejected in production. |
