...
**Owner direction (2026-07-27):** the owner directed that (a) the five minimal operational-readiness items in `docs/37_MINIMAL_OPERATIONAL_READINESS.md` are implemented as the minimal free baseline; (b) the public accessibility statement is incorporated into the site and is **not** a standalone release blocker (a formal specialist accessibility review remains a deferred production item); and (c) mandatory pilot go/no-go criteria are **not** required as a gate before the owner's separate deployment decision. The comprehensive `RF`/`OD` evidence items below remain available as planning aids and as the path to a controlled pilot, but they are not enforced as blockers. On 2026-07-30 the owner provided explicit deploy/release and "deploy now" approval for the self-hosted Keeper replacement on target Ubuntu host `inspiron`; this removes the prior missing-owner-approval deployment gate. Approved wording, minimal readiness, accessibility incorporation, and deploy approval do not weaken required secure-runtime evidence or the implemented real-data controls: borrower application enabled, real-data enabled, and the active consent-catalog row's server-owned `real_data_approved=true` marker for the exact approved consent version/digest.
...
| `OD-04` | Approve the exact ingress implementation, DNS, TLS termination, certificate custody/renewal, trusted proxy behavior, firewall policy, remote-administration path, and whether any service beyond the two public web origins is reachable from outside the host. | On 2026-07-31 the owner selected the existing target-host Nginx/Let's Encrypt setup rather than Caddy. Exact-host routing remains required; its operational configuration and administration boundary are not yet evidenced. | Both |
