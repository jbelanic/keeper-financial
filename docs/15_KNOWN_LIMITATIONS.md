...
- The R2 adapter and hosted Supabase JWT/JWKS path require real non-production integration testing. No provider credentials are included. Secure password reset is delegated to the future managed-identity flow; provider session revocation/offboarding integration is not complete. Candidate MFA policy is now approved in `docs/19_PHASE_1C_CANDIDATE_APPLICATION_POLICY.md` but is not yet implemented for uploads and restricted-document access.
