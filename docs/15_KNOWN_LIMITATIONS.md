...
The current baseline includes the explicitly owner-accepted 2026-07-19 operator-workflow refinement and later forward migrations through the borrower consent wording revision `20260728_0001`. Historical `20260719_0008` migration evidence remains valid for that checkpoint. Completed Git publication did not itself authorize deployment, shared-database migration, production/pilot operation, credential/external-service changes, legal/privacy/regulatory/claims/accessibility approval, or processing of real candidate or borrower data. On 2026-07-30 the owner provided explicit deploy/release and "deploy now" approval for the self-hosted replacement on target host `inspiron`; that approval does not authorize destructive rollback/restore, unsafe credential handling, hosted-service substitution, weakened controls, or skipping the deployed runtime evidence required elsewhere.
...
- The Phase D.2 submit route coordinates the encrypted snapshot, consent evidence, lifecycle transition, retention date, and capability revocation. The owner-approved borrower privacy/credit-use disclosure is versioned as `borrower-privacy-credit-disclosure-2026-07-27-v1` and seeded by forward migration `20260728_0001`.
- Borrower settings are typed in API source, and `.env.example` plus Compose now include borrower feature gates, key-file mounts, document limits, and Docker wiring. Real-data submission still fails closed unless the deployed runtime deliberately enables `BORROWER_APPLICATION_ENABLED=true` and `BORROWER_REAL_DATA_ENABLED=true` and the active consent-catalog row carries the server-owned `real_data_approved=true` marker for the exact approved consent version/digest.
- Key custody/recovery owners, legal-hold authority, public DNS/TLS ceremony, target-host Nginx configuration, backup/restore/purge drill, monitoring/incident response, and public abuse capacity remain operational evidence items during deployment execution rather than missing owner-approval blockers.
