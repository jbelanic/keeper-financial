...
- The candidate remediation introduces forward head `20260717_0006`. A clean isolated PostgreSQL upgrade, head check, downgrade, and re-upgrade passed, and the current local application database was confirmed at `0006` on 2026-07-18. `alembic check` remains non-green only for the pre-existing general Phase 1D index and foreign-key `ondelete` model/schema drift; that diagnostic is separate and no issued migration was rewritten.
...
- The known general Alembic model/schema drift in Phase 1D indexes and foreign-key `ondelete` metadata must be resolved with an approved forward migration or explicitly dispositioned before launch approval. Candidate-remediation migration `20260717_0006` intentionally does not absorb that separate scope.
