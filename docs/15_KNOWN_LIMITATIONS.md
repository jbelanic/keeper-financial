...
- Phase B provides borrower models/migration, typed encrypted draft revisions, capability authorization, SIN masking/reveal primitives, exact-assignment internal authorization, and consent/snapshot/legal-hold schema primitives. Phase C provides the web UI and browser draft client. Phase D.1 provides document upload, immutable encrypted snapshot creation, consent binding, and capability revocation. None of these phases provides internal queue/review UI breadth, download/review operations, purge/legal-hold operations, or dedicated TLS ingress.
