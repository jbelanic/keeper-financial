...
def save_draft_payload(
    ...
    has_sin = bool(payload_data.get("primary_borrower", {}).get("sin"))
    has_co_borrower = bool(payload_data.get("co_borrower"))

    encrypted_sin_ciphertext = None
    encrypted_sin_nonce = None
    sin = payload_data.get("primary_borrower", {}).get("sin")
    if sin:
        from keeper_api.services.borrower_crypto import encrypt_sin

        encrypted_sin_ciphertext, encrypted_sin_nonce = encrypt_sin(
            crypto_state, sin, str(application_id), expected_revision + 1
        )

    payload_for_encryption = {k: v for k, v in payload_data.items() if k != "primary_borrower"}
    if "primary_borrower" in payload_data:
        borrower_data = dict(payload_data["primary_borrower"])
        ...
        if existing_payload is not None:
            ...
            try:
                ...
                envelope = EncryptedEnvelope(
                    format_version=1,
                    key_id=existing_payload.key_id,
                    nonce=existing_payload.nonce,
                    ciphertext=existing_payload.ciphertext,
                )
                existing_plaintext = decrypt_payload(
                    state=crypto_state,
                    envelope=envelope,
                    application_id=str(application_id),
                    purpose="borrower_application",
                    schema_version=existing_payload.schema_version,
                    payload_revision=existing_payload.revision,
                )
